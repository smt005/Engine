...
void MapPhysX::init()
{
	if (!load())
	{
		...
		Camera::current.setPos(glm::vec3(30.0f, 27.0f, 24.0f));
		Camera::current.setVector(glm::vec3(-0.7f, -0.6f, -0.3f));
...
{
	if (!_callback) {
		_callback = new Engine::Callback(Engine::CallbackType::PINCH_TAP, [this](const Engine::CallbackEventPtr& callbackEventPtr) {
			...
			if (Engine::Callback::pressTap(Engine::VirtualTap::LEFT)) {
				glm::vec3 vecForce = Camera::current.vector();
				glm::vec3 posCamear = Camera::current.pos() + vecForce;
				vecForce = vecForce * 25.0f;

				Object& newObject = _mapGame->addObjectToPos("SphereFire", posCamear);
				newObject.tag = 10;
				newObject.setTypeActorPhysics(Engine::Physics::Type::CONVEX);
				if (newObject.createActorPhysics()) {
					newObject.addForce(vecForce, Engine::Physics::Force::IMPULSE);
				}
			}
		});
...
void MapPhysX::update()
{
	...
	//...
	std::vector<Object*> needRemoveObjects;
	for (Object* objectPtr : _mapGame->objects) {
		if (objectPtr->tag == 10) {
			double currentTime = Engine::Core::currentTime();
			double deadTime = objectPtr->getCreateTime() + 10000;
			if (deadTime < currentTime) {
				needRemoveObjects.emplace_back(objectPtr);
			}
		}
	}

	for (Object* removedObjectPtr : needRemoveObjects) {
		auto it = std::find_if(_mapGame->objects.begin(), _mapGame->objects.end(), [removedObjectPtr](const Object* objectPtr) { return removedObjectPtr == objectPtr; });
		if (it != _mapGame->objects.end()) {
			(*it)->releaseActorPhysics();
			delete* it;
			_mapGame->objects.erase(it);
		}
	}
}
