...
//...
unsigned int MakeTexture(uint8_t * image, int32_t width, int32_t height);
...
unsigned int FreeType2Main(const std::string& text)
{
    ...
    unsigned int idTexture = MakeTexture(image.data(), imageW, imageH);
...
unsigned int MakeTexture(uint8_t * image, int32_t width, int32_t height) {
    ...
    for (int col = 0; col < width; ++col) {
        for (int row = 0; row < height; ++row) {
            ...
            indexNew = index * 4;
            unsigned char value = (unsigned char)image[index];
