...
#include "CameraControlOutside.h"
#include "Core.h"
#include "Callback/Callback.h"
#include "Callback/CallbackEvent.h"

CameraControlOutside::~CameraControlOutside() {
	if (_callbackPtr) {
		delete _callbackPtr;
...
// VIRTUAL
void CameraControlOutside::Load(const Json::Value& data) {
	Camera::Load(data);
...
void CameraControlOutside::Save(Json::Value& data) {
	Camera::Save(data);

	data["class"] = "CameraControlOutside";
	data["speed"] = _speed;
	data["angleSpeed"] = _angleSpeed;
}

//...
void CameraControlOutside::Enable(const bool state) {
	if (state) {
		if (!_callbackPtr) {
			MakeCallback();
		}
...
template <typename T>
void CameraControlOutside::Move(const T& directVector, const float kForce) {
	glm::vec3 pos = Pos();
	pos += directVector.x * _speed * kForce;
	SetPos(pos);
}

void CameraControlOutside::Rotate(const glm::vec2& angles) {
...
void CameraControlOutside::SetPosOutside(const glm::vec3& posOutside, const float distOutside) {
...
void CameraControlOutside::SetPosOutside(const glm::vec3& posOutside) {
	SetPosOutside(posOutside, _distOutside);
}

void CameraControlOutside::SetDistanceOutside(const float distOutside) {
