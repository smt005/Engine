...
// D:\CMake\CMakeProject\Source\Engine\ThirdParty\FreeImage\Source\LibPNG
//#include <png.h>
...
namespace FreeType2 {
    ...
    int main(const std::string& text, const std::string& Font, const unsigned int size)
    {
        ...
        FT_Face ftFace = 0;
        std::string filePathFont = "D:/CMake/CMakeProject/Source/Resources/Files/Fonts/" + Font + ".ttf";
        FT_New_Face(ftLibrary, filePathFont.c_str(), 0, &ftFace);

        // Установим размер символа для рендеринга
        FT_Set_Pixel_Sizes(ftFace, (size * 100), 0);

        // Выводимая строка
        const std::string exampleString = text;
    ...
    void savePNG(uint8_t* image, int32_t width, int32_t height)
    {
        std::string outPutFile = "D:/CMake/output" + std::to_string(index) + ".png";
        ++index;
...
/*
#include "Draw/TestFreeType2.h"
#include <string>

int main(int argc, char** argv)
{
    FreeType2::main("GOST type A Standard GOST type A Standard GOST type A Standard", "Futured", 1);
    FreeType2::main("GOST type A Standard GOST type A Standard GOST type A Standard", "GOST type A Standard", 1);

    FreeType2::main("GOST type A Standard GOST type A Standard GOST type A Standard", "Futured", 5);
    FreeType2::main("GOST type A Standard GOST type A Standard GOST type A Standard", "GOST type A Standard", 5);

    FreeType2::main("Futured", "Futured", 1);
    FreeType2::main("Futured", "GOST type A Standard", 1);

    FreeType2::main("Futured", "Futured", 5);
    FreeType2::main("Futured", "GOST type A Standard", 5);
